...
if options.sentinel=='S2':
    query_slc='%s AND (platformname:Sentinel-2)'%(query_orb)
elif options.sentinel=='S1':
    query_slc='%s AND (platformname:Sentinel-1)'%(query_orb)
else:
    query_slc = query_orb
...
query_time='%s AND (beginPosition:[%sT00:00:00.000Z TO %sT23:59:59.999Z] AND endPosition:[%sT00:00:00.000Z TO %sT23:59:59.999Z])'%(query_slc,start_date,end_date,start_date,end_date)

#cloud cover query
if options.max_cloud!=None:
    query='%s AND  (cloudcoverpercentage:[0.0 TO %s])'%(query_time, options.max_cloud/100)
else:
    query=query_time
...
#use this part if you want to have your password and username saved in a textfile
...
### FIX MULTIPLICATION OF MB vs. GB
...
if messagebox == True:
    ...
    for entry in range(len(entries)):
        ...
        #if write_dir defined, save there, otherwise save to folder where the python script is located
