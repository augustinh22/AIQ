...
def get_args():
    ...
    if len(sys.argv) == 1:
        ...
        parser.add_argument(
            '-b8', '--bit8', dest='bit8', action='store', type=str,
            help='If 8bit is to be warped.', default=None)
        parser.add_argument(
...
def get_procFolders(options_in):
    ...
    for dirpath, dirnames, filenames in os.walk(
        ...
        for dirname in dirnames:
            ...
            if dirname == 'PROC_DATA':
                ...
                procFolders.append(procFolder)
    #
    # Determine which tile folders have calrefbyt files.
    #
    unprocFolders = []
    ...
    for procFolder in procFolders:
        ...
        calrefbyt_path = None
        calrefbyt_size = 0
        keep_procFolder_1 = None
        keep_procFolder_2 = None
        ...
        for filename in os.listdir(procFolder):
            ...
            if filename.endswith('caltembyt_lndstlk.dat'):
                ...
                if calrefbyt_size > 5:
                    keep_procFolder_2 = True

        #
        # Removes empty PROC_DATA folders with problem files and adds to list to be
        # processed.
        #
        if keep_procFolder_1 is True and keep_procFolder_2 is True:
    ...
    #
    question = (
        'Number of proc tiles found: {}'
        '\n\nDo you want to process all unprocessed folders [{}]?'
        ).format(len(procFolders), len(unprocFolders))
...
def warp_envi(root_folder, procFolders, warp_epsg, delete_option):
    ...
    print '================================================================='
    print 'Hold on to your hat. This may take ~45s per S2 tile folder.'
    print 'Number of unprocessed PROC_DATA folders found: {}'.format(
        len(procFolders))
    print 'Estimated time: {} minutes'.format(int(len(procFolders)) * 0.75)
    print 'Start time: {}'.format(start_time.time())
    print '=================================================================\n'
    ...
    message = (
        'Root Folder: {} \nNumber of unprocessed IMG_DATA folders '
        'found: {}\nStart time: {}'
        ).format(root_folder, len(procFolders), start_time.time())
    logger.info(message)
    #
    # Register all of the GDAL drivers.
    #
    ...
    for procFolder in procFolders:
        ...
        for filename in os.listdir(procFolder):
            ...
            if filename.endswith('caltembyt_lndstlk.dat'):
                ...
                caltembyt_path = os.path.join(procFolder, filename)
                files_to_warp.append(caltembyt_path)
...
if __name__ == '__main__':
    ...
    if options.bit8 is None:
        ...
        bool_answer, procFolders_toProcess = get_procFolders(options)

        if not bool_answer:
            print 'No folders processed.'
            sys.exit(1)

        if procFolders_toProcess == 0:
